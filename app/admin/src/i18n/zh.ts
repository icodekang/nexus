/**
 * @file Admin Chinese Translations - 管理员端中文翻译
 * 包含管理后台所有界面的中文文本
 */

export const zh = {
  common: {
    brandName: 'Nexus',
    menu: '菜单',
    admin: '管理',
    administrator: '管理员',
    signOut: '退出登录',
    edit: '编辑',
    delete: '删除',
    active: '活跃',
    inactive: '未活跃',
    close: '关闭',
    save: '保存',
    cancel: '取消',
    confirm: '确认',
    loading: '加载中...',
    viewDetails: '查看详情',
    deleteConfirm: '确定要删除此项吗？',
    prev: '上一页',
    next: '下一页',
  },
  errors: {
    // Auth errors
    invalid_credentials: '邮箱或密码错误，请重试。',
    unauthorized: '请登录后继续操作。',
    forbidden: '权限不足，仅管理员可登录。',
    // Network errors
    network_error: '网络错误，请检查网络连接后重试。',
    request_failed: '请求失败，请重试。',
    internal_error: '服务器错误，请稍后重试。',
    // CRUD errors
    create_failed: '创建失败，请重试。',
    update_failed: '更新失败，请重试。',
    delete_failed: '删除失败，请重试。',
    // Browser account errors
    qr_generate_failed: '认证失败，请重试。',
    auth_failed: '认证失败，请重试。',
    account_add_failed: '添加账号失败，请重试。',
    account_delete_failed: '删除账号失败，请重试。',
    // Generic
    try_again: '出现错误，请重试。',
    contact_support: '如果问题持续存在，请联系客服。',
  },
  login: {
    title: '欢迎回来',
    subtitle: '登录 Nexus 管理后台',
    email: '邮箱',
    password: '密码',
    emailPlaceholder: 'admin@nexus.io',
    passwordPlaceholder: '请输入密码',
    signingIn: '登录中...',
    signIn: '登录',
    invalidCredentials: '邮箱或密码错误，请重试。',
    networkError: '网络错误，请检查网络连接后重试。',
    serverError: '服务器错误，请稍后重试。',
    forbidden: '权限不足，仅管理员可登录。',
    footer: 'Nexus 平台 v1.0',
  },
  sidebar: {
    dashboard: '仪表盘',
    users: '用户',
    providers: '服务商',
    providerKeys: '密钥管理',
    models: '模型',
    transactions: '交易记录',
    browserAccounts: '浏览器账号',
  },
  dashboard: {
    title: '仪表盘',
    subtitle: '平台概览与关键指标',
    last30Days: '最近 30 天',
    last7Days: '最近 7 天',
    totalUsers: '用户总数',
    activeSubscriptions: '活跃订阅',
    monthlyRevenue: '月收入',
    apiCallsToday: '今日 API 调用',
    revenueOverview: '收入概览',
    recentActivity: '最近活动',
    purchasedPlan: '购买了 {{plan}} 套餐',
    madeApiCall: '进行了 API 调用',
    upgradedTo: '升级到 {{plan}}',
    subscriptionExpired: '订阅已过期',
    timeAgo: '{{time}}前',
  },
  users: {
    title: '用户',
    count: '共 {{count}} 个用户',
    searchPlaceholder: '搜索用户...',
    addUser: '添加用户',
    editUser: '编辑用户',
    thUser: '用户',
    thPhone: '手机',
    thPlan: '套餐',
    thStatus: '状态',
    thCreated: '创建时间',
    emailLabel: '邮箱',
    emailPlaceholder: 'user@example.com',
    phoneLabel: '手机',
    phonePlaceholder: '+86 138xxxx8888',
    planLabel: '套餐',
    statusLabel: '状态',
    selectPlan: '选择套餐',
    selectStatus: '选择状态',
    pageInfo: '第 {{page}} 页，共 {{totalPages}} 页（共 {{total}} 条）',
  },
  providers: {
    title: '服务商',
    subtitle: '管理 LLM 服务商及其配置',
    addProvider: '添加服务商',
    editProvider: '编辑服务商',
    models: '模型',
    priority: '优先级',
    nameLabel: '名称',
    namePlaceholder: '例如 OpenAI',
    slugLabel: '标识',
    slugPlaceholder: '例如 openai',
    apiKeyLabel: 'API 密钥',
    apiKeyPlaceholder: 'sk-...',
    priorityLabel: '优先级',
    deleteErrorTitle: '无法删除服务商',
    deleteErrorKeys: '该服务商仍有有效的 API 密钥。请先在「密钥管理」页面删除所有密钥。',
    deleteErrorAccounts: '该服务商仍有有效的浏览器账号。请先在「浏览器账号」页面删除所有账号。',
    deleteErrorBoth: '该服务商仍有有效的 API 密钥和浏览器账号。请先在「密钥管理」和「浏览器账号」页面删除所有密钥和账号。',
    deleteErrorModels: '该服务商仍有模型配置。请先在「模型」页面删除所有模型。',
  },
  models: {
    title: '模型',
    subtitle: '所有服务商共 {{count}} 个可用模型',
    addModel: '添加模型',
    editModel: '编辑模型',
    thModel: '模型',
    thProvider: '服务商',
    thContext: '上下文',
    thCapabilities: '能力',
    nameLabel: '模型名称',
    namePlaceholder: '例如 GPT-4o',
    providerLabel: '服务商',
    selectProvider: '选择服务商',
    contextLabel: '上下文窗口',
    contextPlaceholder: '例如 128K',
    capabilitiesLabel: '能力',
    capsPlaceholder: 'vision, function（逗号分隔）',
  },
  transactions: {
    title: '交易记录',
    subtitle: '收入与付款历史',
    allTypes: '全部类型',
    allStatus: '全部状态',
    purchase: '购买',
    refund: '退款',
    renewal: '续费',
    completed: '已完成',
    refunded: '已退款',
    pending: '待处理',
    revenueToday: '今日收入',
    transactionsCount: '交易笔数',
    avgOrder: '平均订单',
    thUser: '用户',
    thType: '类型',
    thPlan: '套餐',
    thAmount: '金额',
    thStatus: '状态',
    thDate: '日期',
    thActions: '操作',
    detailTitle: '交易详情',
    detailId: '交易 ID',
    detailUser: '用户',
    detailType: '类型',
    detailPlan: '套餐',
    detailAmount: '金额',
    detailStatus: '状态',
    detailDate: '日期',
    refundAction: '退款',
    refundConfirm: '确定要退款此交易吗？',
    pageInfo: '第 {{page}} 页，共 {{totalPages}} 页（共 {{total}} 条）',
  },
  providerKeys: {
    title: '密钥管理',
    subtitle: '管理各 LLM 服务商的 API 密钥',
    addKey: '添加密钥',
    editKey: '编辑密钥',
    providerLabel: '服务商',
    selectProvider: '选择服务商',
    apiKeyLabel: 'API 密钥',
    apiKeyPlaceholder: 'sk-...',
    baseUrlLabel: '基础 URL',
    baseUrlPlaceholder: 'https://api.example.com/v1',
    priorityLabel: '优先级',
    testKey: '测试',
    testing: '测试中...',
    testSuccess: '连接成功',
    testFailed: '连接失败',
    maskedKey: '密钥已隐藏，保护安全',
    revealKey: '显示',
    hideKey: '隐藏',
    thProvider: '服务商',
    thKey: 'API 密钥',
    thUrl: '基础 URL',
    thStatus: '状态',
    thPriority: '优先级',
    thActions: '操作',
    noKeys: '暂未配置服务商密钥',
    noKeysDesc: '添加 LLM 服务商的 API 密钥以启用模型路由。',
    noProviders: '请先添加服务商',
    addProviderFirst: '请先在「服务商管理」中添加服务商',
    tabProvider: '服务商密钥',
    tabUser: '用户密钥',
    filterByEmail: '按用户邮箱搜索...',
    thUser: '用户',
    thKeyName: '密钥名称',
    thLastUsed: '最后使用',
    thCreated: '创建时间',
    unnamed: '未命名',
    noUserKeys: '暂无用户密钥',
    noUserKeysDesc: '用户创建 API 密钥后将显示在这里。',
    deleteUserKeyConfirm: '确定要删除此用户密钥吗？此操作不可撤销。',
  },
  browserAccounts: {
    title: '浏览器账号',
    subtitle: '已配置 {{count}} 个账号',
    addClaude: '添加 Claude 账号',
    addChatGPT: '添加 ChatGPT 账号',
    email: '邮箱',
    requests: '请求次数',
    lastUsed: '最后使用',
    noAccounts: '暂无浏览器账号',
    noAccountsDesc: '添加浏览器账号并通过 Cookie 注入完成认证。',
    noProviders: '暂无支持浏览器的服务商',
    goProviders: '去服务商页面添加大模型服务商（支持 OpenAI、Anthropic、DeepSeek）',
    deleteConfirmTitle: '删除账号',
    deleteConfirm: '确定要删除此浏览器账号吗？此操作不可撤销。',
    accountCountLabel: '个账号',
    unnamed: '未命名账号',
    cookieInject: 'Cookie 注入',
    addAccount: '添加账号',
    accountName: '账号名称',
    accountNamePlaceholder: '例如：我的 ChatGPT 账号',
    nameRequired: '请输入账号名称',
    status: {
      pending: '待认证',
      active: '已激活',
      expired: '已过期',
      error: '错误',
    },
    passwordLogin: '账号密码登录',
  },
  loginModal: {
    title: '账号认证',
    browserLoginTitle: '浏览器登录',
    browserLoginDesc: '打开浏览器窗口，手动登录后自动捕获 Cookie',
    openBrowserLogin: '打开浏览器登录',
    recommended: '推荐',
    active: '进行中',
    waitingForLogin: '请在浏览器窗口中完成登录...',
    browserLoginHint: '登录成功后系统将自动捕获 Cookie 并关闭窗口',
    browserLoginFailed: '登录失败或超时',
    retry: '重试',
    advancedOptions: '高级选项',
    manualInjectTitle: '手动注入 Cookie',
    autoLoginTitle: '自动登录',
    step1Title: '在浏览器中登录',
    step1Desc: '在 Chrome/Safari 中打开 {{provider}} 并登录。',
    step2Title: '导出 Cookie',
    step3Title: '粘贴到下方',
    step3Desc: '粘贴 Cookie JSON 后点击"注入会话"。',
    cookieJsonLabel: 'Cookie JSON',
    pasteFromClipboard: '从剪贴板粘贴',
    injectBtn: '注入会话',
    clipboardError: '无法访问剪贴板，请手动粘贴。',
    jsonError: 'JSON 格式无效，请检查后重试。',
    successTitle: '认证完成',
    successRedirect: '跳转中...',
    autoToggleShow: '尝试自动登录 →',
    autoToggleHide: '收起',
    autoDivider: '或尝试自动登录（主流平台可能不适用）',
    autoPasswordTab: '密码',
    autoPhoneTab: '手机验证码',
    autoEmail: '邮箱',
    autoPassword: '密码',
    autoLoginBtn: '自动登录',
    autoSendCode: '发送验证码',
    autoSentTo: '已发送至 {{phone}}',
    autoChangePhone: '修改手机号',
    autoVerify: '验证登录',
  },
  lang: {
    label: '中文',
  },
};
