...
pub async fn validate_api_key(mut req: Request, next: Next) -> Result<Response, ApiError> {
    let auth_header = req
        .headers()
        .get(AUTHORIZATION)
    ...
    let (api_key, user) = validator.validate(key).await.map_err(|e| match e {
...
pub async fn validate_jwt_or_api_key(mut req: Request, next: Next) -> Result<Response, ApiError> {
